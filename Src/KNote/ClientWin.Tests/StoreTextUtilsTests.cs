...
public class StoreTextUtilsTests
{
    ...
    {
        var result = _store.TextToDateTime("2026-08-23");
    ...
    {
        Assert.IsNull(_store.TextToDateTime("not a date"));
    }

    [TestMethod]
    public void TextToInt_ValidNumber_ReturnsParsedValue()
    {
        Assert.AreEqual(42, _store.TextToInt("42"));
    }

    [TestMethod]
    public void TextToInt_InvalidText_ReturnsZero()
    {
        Assert.AreEqual(0, _store.TextToInt("not a number"));
    }
    ...
    public void TextToDouble_ValidNumber_ReturnsParsedValue()
    {
        ...
        Assert.AreEqual(3.14, _store.TextToDouble(numberText));
    }

    [TestMethod]
    public void TextToDouble_InvalidText_ReturnsNull()
    {
        Assert.IsNull(_store.TextToDouble("not a number"));
    }

    [TestMethod]
    public void ExtractUrlFromText_SingleLineHttpUrl_ReturnsUrl()
    {
        Assert.AreEqual("https://example.com", _store.ExtractUrlFromText("https://example.com"));
    }

    [TestMethod]
    public void ExtractUrlFromText_UrlOnFirstLineOfMultilineText_ReturnsFirstLine()
    {
        var result = _store.ExtractUrlFromText("https://example.com\nsome other text");
    ...
    {
        Assert.IsNull(_store.ExtractUrlFromText("this is just a note"));
    }

    [TestMethod]
    public void ExtractUrlFromText_EmptyText_ReturnsNull()
    {
        Assert.IsNull(_store.ExtractUrlFromText(""));
    }
    ...
    {
        Assert.AreEqual(expectedMimeType, _store.ExtensionFileToFileType(extension));
    }

    [TestMethod]
    public void ExtensionFileToFileType_IsCaseInsensitive()
    {
        Assert.AreEqual("application/pdf", _store.ExtensionFileToFileType(".PDF"));
    }
    ...
    {
        Assert.AreEqual(expected, _store.IsSupportedFileTypeForPreview(fileType));
    }
}
