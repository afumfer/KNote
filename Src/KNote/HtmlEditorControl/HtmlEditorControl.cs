...
using mstmlSelection = mshtml.IHTMLSelectionObject;
using mstmlControlRange = mshtml.IHTMLControlRange;

using mshtmlElement = mshtml.IHTMLElement;
using mstmlElementCollection = mshtml.IHTMLElementCollection;
using mshtmlControlElement = mshtml.IHTMLControlElement;
using mstmlAnchorElement = mshtml.IHTMLAnchorElement;
using mshtmlImageElement = mshtml.IHTMLImgElement;
using mstmlFontElement= mshtml.IHTMLFontElement;
...
namespace MSDN.Html.Editor
{
	...
	public sealed partial class HtmlEditorControl : UserControl
	{
        ...
        private void BrowserBeforeNavigate(object sender, WebBrowserNavigatingEventArgs e)
        {
			...
			{
				// TODO: (A) Should ensure the following are no executed for the editor navigation
				//   Scripts
				//   Java
				//   ActiveX Controls
				//   Behaviors
        ...
        {			
            ...
            // muestra un pdf desde ANotas.
            if (this.editorWebBrowser.Document == null)
            {                
                return;
            }
					...
					if (result == DialogResult.Yes)
					{
						...
						if (IsValidHref(hrefLink))
						{
							...
							if (anchor == null)
							{
								...
								if (element != null) anchor = (mstmlAnchorElement)element;
							}
							else
							{
        ...
        public void InsertStyleSource(string styleSource)
        {
            if (styleSource != null)
            {
                ...
                styleNode = bodyNode.appendChild(styleNode);
                styleNode = styleNode.swapNode(bodyNode.firstChild);
        ...
        public void InsertScriptSource(string scriptSource)
        {
            if (scriptSource != null)
            {
                ...
                scriptNode = bodyNode.appendChild(scriptNode);
		...
		public void FindReplaceReset()
		{
			...
			((mstmlSelection)document.selection).empty();
		...
		private void ProcessTable(mshtmlTable table, HtmlTableProperty tableProperties)
		{
			try
			{
				...
				else
				{
					...
					mstmlElementCollection rows = table.rows;
				...
				if (tableCreated)
				{
					...
					mstmlSelection selection = document.selection;
					...
					{
						mstmlControlRange controlRange = GetAllControls();
        ...
        /// </summary>
		public mshtmlTextRange GetTextRange()
		{
			// define the selected range object
			mstmlSelection selection;
		...
		private mshtmlElement GetFirstControl()
		{
			...
			try
			{
				...
				{
					range = (mstmlControlRange)selection.createRange();
        ...
        /// </summary>
		/// <returns></returns>
		private mstmlControlRange GetAllControls()
		{
			// define the selected range object
			mstmlSelection selection;
			mstmlControlRange range = null;
		...
		private void RebaseAnchorUrl()
		{
			if (rebaseUrlsNeeded)
			{
				...
				mstmlElementCollection anchors = body.getElementsByTagName(ANCHOR_TAG);
				foreach (mshtmlElement element in anchors)
				{
					try
					{
						mstmlAnchorElement anchor = (mstmlAnchorElement)element;
        ...
        private void SetBrowserPanelSize()
        {
            ...
            if (_toolbarDock == DockStyle.Top || _toolbarDock == DockStyle.Bottom)
            {
                ...
                this.browserPanel.Height = (_toolbarVisible) ? Math.Max(0, this.Height - this.toolstripEditor.Height - 2) : this.Height - 2;
            }
